...
combined.data <- read_csv(input.file) %>% #set reference values for each group
  ...
  mutate(BMI_cat = relevel(as.factor(BMI_cat),ref="Normal"))%>%
...
combined.data %>%
  ...
  rename("Yes"="Diabetes1",
         "No"="Diabetes0")%>%
  mutate(Prevalence=Yes/(Yes+No)) -> diabetes.race
...
combined.data %>%
  ...
  rename("Yes"="Diabetes1",
         "No"="Diabetes0")%>%
  mutate(Prevalence=Yes/(Yes+No)) -> diabetes.age
...
combined.data %>%
  ...
  rename("Yes"="Diabetes1",
         "No"="Diabetes0") %>%
  mutate(Prevalence=Yes/(Yes+No)) -> diabetes.bmi
