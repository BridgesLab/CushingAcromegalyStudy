...
# Add interaction term as separate row or column as you prefer:
# simplest: keep it separate and rbind across outcomes later
# e.g. interaction_row.glucose
...
```{r ldl-statistics}
#used fully adjusted model
lm.ldl <- lm(value ~ RaceEthnicity + AgeInYears + GenderName +                     Cushings * Obesity, data = ldl.data)
...
```{r map-statistics}
#used fully adjusted model
lm.map <- lm(BPMeanNonInvasive ~ RaceEthnicity + AgeInYears + GenderName +                     Cushings * Obesity, data = map.data)
...
map.summary %>% kable(caption="Summary of Mean Arterial Pressure levels by obesity and pre-existing Cushing's")
lm(value ~ Cushings + Obesity + Cushings:Obesity,data=map.data) |> 
  tidy() |> 
  kable(caption="2x2 ANOVA with Interaction for effects of Obesity and Cushings on Mean Arterial Pressure")

lm(value ~ Cushings + BMI + Cushings:Obesity,data=map.data) |> 
  tidy() |> 
  kable(caption="2x2 ANOVA with Interaction for effects of Obesity and BMI on Mean Arterial Pressure")
```
...
```{r sbp-statistics}
#used fully adjusted model
lm.sbp <- lm(BPMeanNonInvasive ~ RaceEthnicity + AgeInYears + GenderName +                     Cushings * Obesity, data = sbp.data)

## 1. Adjusted means
emm.sbp <- emmeans(lm.sbp, ~ Cushings | Obesity) %>%
  as.data.frame() %>%
  mutate(
    Outcome = "Systolic Arterial Pressure",
    Mean_CI = sprintf("%.2f (%.2f–%.2f)", emmean, lower.CL, upper.CL)
  ) %>%
  select(Outcome, Obesity, Cushings, Mean_CI)
...
sbp.summary %>% kable(caption="Summary of Systolic Arterial Pressure levels by obesity and pre-existing Cushing's")
lm(value ~ Cushings + Obesity + Cushings:Obesity,data=sbp.data) |> 
  tidy() |> 
  kable(caption="2x2 ANOVA with Interaction for effects of Obesity and Cushings on Systolic Arterial Pressure")

lm(value ~ Cushings + BMI + Cushings:Obesity,data=sbp.data) |> 
  tidy() |> 
  kable(caption="2x2 ANOVA with Interaction for effects of Obesity and BMI on Systolic Arterial Pressure")
```
...
```{r dbp-statistics}
#used fully adjusted model
lm.dbp <- lm(BPMeanNonInvasive ~ RaceEthnicity + AgeInYears + GenderName +                     Cushings * Obesity, data = dbp.data)

## 1. Adjusted means
emm.dbp <- emmeans(lm.dbp, ~ Cushings | Obesity) %>%
  as.data.frame() %>%
  mutate(
    Outcome = "Diastolic Arterial Pressure",
    Mean_CI = sprintf("%.2f (%.2f–%.2f)", emmean, lower.CL, upper.CL)
  ) %>%
  select(Outcome, Obesity, Cushings, Mean_CI)
...
dbp.summary %>% kable(caption="Summary of Diastolic Arterial Pressure levels by obesity and pre-existing Cushing's")
lm(value ~ Cushings + Obesity + Cushings:Obesity,data=dbp.data) |> 
  tidy() |> 
  kable(caption="2x2 ANOVA with Interaction for effects of Obesity and Cushings on Diastolic Arterial Pressure")

lm(value ~ Cushings + BMI + Cushings:Obesity,data=dbp.data) |> 
  tidy() |> 
  kable(caption="2x2 ANOVA with Interaction for effects of Obesity and BMI on Diastolic Arterial Pressure")
```
