...
We will use the last BMI measure if there are multiple in the encounters file.
...
bmi.data.last <- 
  bmi.data %>%
  group_by(DeID_PatientID) %>%
  summarize(BMI = last(BMI,na.rm=T),
            BMI.n = length(BMI)) 
...
Based on this procedure we had `r nrow(bmi.data.last)` participants in the initial dataset.  After removing individuals with no BMI measure we had `r nrow(bmi.data.complete)` (a loss of `r nrow(bmi.data.last)-nrow(bmi.data.complete)` participants).  We then removed anyone who's last BMI was >`r bmi.cutoff` or <`r bmi.lower`, a loss of `r nrow(bmi.data.complete)-nrow(bmi.data.cutoff)` participants.  This resulted in a final dataset of `r nrow(bmi.data.cutoff)`.
...
We filtered out all BMI that the last was \>`r bmi.cutoff`. After this the highest and lowest BMI were
...
bmi.data.cutoff %>%
  ...
  select(BMI,BMI.n) %>%
  head(10) %>%
  kable(caption="Top 50 last BMI values and number of encounters")
