...
filename <- "../data/processed/Normalized Counts Table.csv"
...
library(reshape2)
filtered.normalized.data$Transcript <- rownames(filtered.normalized.data)
#melt the data, so that each transcript/sample has one value
filtered.normalized.melted.data <- melt(filtered.normalized.data[gene.data$ensembl_transcript_id,], variable.name="samplename", id.vars="Transcript")
#annotate that data frame with the gene name
normalized.melted.data.full <- merge(filtered.normalized.melted.data, gene.data, by.x="Transcript", by.y="ensembl_transcript_id", all.x=T)
...
#melt the data, so that each transcript/sample has one value
normalized.melted.lipase.data <- melt(as.data.frame(filtered.normalized.data[merged.lipase.data$ensembl_transcript_id,]), variable.name="samplename", id.vars="Transcript")
#annotate that data frame with the gene name
