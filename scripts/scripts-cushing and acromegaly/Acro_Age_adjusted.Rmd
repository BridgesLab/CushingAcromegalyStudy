...
acromegaly.mapping$age.cat <- cut(acromegaly.mapping$age, breaks=c(0,60,100))
...
g <-ggplot (acro.means.long, aes (age, value, colour = group)) +geom_point(size=3)+
  geom_vline(xintercept=c(60),linetype="dashed", colour="green")+
...
cds.acromegaly.age$agexGroup <-relevel(cds.acromegaly.age$agexGroup, ref="Control.(0,60]")
...
cds.acromegaly$age.cat <- relevel(cds.acromegaly$age.cat, ref="(0,60]")
...
### compare Acro and Control in the age group 0 to 60
acro.age.res$log2FC_AcrovsCon_0.60 <- results(acro.cds.age, name="agexGroup_Acromegaly..0.60._vs_Control..0.60.")$log2FoldChange
acro.age.res$lfcSE_AcrovsCon_0.60 <- results(acro.cds.age, name="agexGroup_Acromegaly..0.60._vs_Control..0.60.")$lfcSE
acro.age.res$pavlue_AcrovsCon_0.60 <- results(acro.cds.age, name="agexGroup_Acromegaly..0.60._vs_Control..0.60.")$pvalue
acro.age.res$padj_AcrovsCon_0.60 <- results(acro.cds.age, name="agexGroup_Acromegaly..0.60._vs_Control..0.60.")$padj
...
### compare Old vs Young in the Control group
acro.age.res$log2FC_OldvsYoung_Con <- acro.age.res$log2FoldChange
acro.age.res$lfcSE_OldvsYoung_Con <- acro.age.res$lfcSE
acro.age.res$pvalue_OldvsYoung_Con <- acro.age.res$pvalue
acro.age.res$padj_OldvsYoung_Con <- acro.age.res$padj

### compare Old vs Young in the Acromegaly group
acro.age.res$log2FC_OldvsYoung_Acro <- results(acro.cds.age, contrast=c("agexGroup","Acromegaly..60.100.","Acromegaly..0.60."))$log2FoldChange
acro.age.res$lfcSE_OldvsYoung_Acro <- results(acro.cds.age, contrast=c("agexGroup","Acromegaly..60.100.","Acromegaly..0.60."))$lfcSE
acro.age.res$pvalue_OldvsYoung_Acro <- results(acro.cds.age, contrast=c("agexGroup","Acromegaly..60.100.","Acromegaly..0.60."))$pvalue
acro.age.res$padj_OldvsYoung_Acro <- results(acro.cds.age, contrast=c("agexGroup","Acromegaly..60.100.","Acromegaly..0.60."))$padj
...
sum(acro.age.res$padj_AcrovsCon_0.60<0.05, na.rm=TRUE)
acro.age.res<- acro.age.res[order(acro.age.res$padj_AcrovsCon_0.60),]
...
sig.060 <- subset(acro.age.Wald, padj_AcrovsCon_0.60<0.05)
sig.060 <- sig.060$external_gene_id
...
write.csv(sig.060, "../data/processed/Acro_vs_Control_Young_Sig_Genes.csv")
```
Adjusting for age
----------------
Acromegaly: The number of significant genes affected by disease controlling for all age groups is `r sum(acro.res$padj<0.05, na.rm=TRUE)`. Number of genes significantly changed between Acromegaly and Control in the age group (0,60] is `r sum(acro.age.res$padj_AcrovsCon_0.60<0.05, na.rm=TRUE)`. Number of genes significantly changed between Acromegaly and Control in the age group (60,100] is `r sum(acro.age.res$padj_AcrovsCon_60.100<0.05, na.rm=TRUE)`.
