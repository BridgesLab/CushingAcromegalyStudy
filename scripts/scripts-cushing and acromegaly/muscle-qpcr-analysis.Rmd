...
library(car)
genes.of.interest <- c('Fbxo32','Trim63','Psmd8')
...
for (gene in genes.of.interest){
  if (stats.summary[gene,'shapiro-dex'] < 0.05) {
...
plot <- barplot(as.matrix(dcast(qpcr.summary, Treatment~Gene, value.var='norm.mean')[2:(length(levels(data$Gene))+1)]), 
        beside=T, las=2, ylim=c(0,ymax), ylab="Relative Expression")

superpose.eb(plot,
             as.matrix(dcast(qpcr.summary, Treatment~Gene, value.var='norm.mean')[2:(length(levels(data$Gene))+1)]),
             as.matrix(dcast(qpcr.summary, Treatment~Gene, value.var='norm.se')[2:(length(levels(data$Gene))+1)]))
legend('topleft', levels(data$Treatment), bty='n', fill=grey.colors(2))
```
