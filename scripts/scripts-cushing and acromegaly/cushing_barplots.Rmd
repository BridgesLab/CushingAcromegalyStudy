...
mapping$group <- relevel(as.factor(mapping$group), ref="non-functioning")
...
#paste gene here
genes.of.interest <- c("NR3C1","NR3C2","KLF5","CDKN2B","LEP","ADIPOQ","FAM47E-STBD1","STBD1")
...
for (gene in genes.of.interest) {
  ...
  ggsave(filename=paste('../../data/Figures/Cushing-',gene,'-barplot.pdf',sep=""))
}
...
lipases <- c("LPL","PNPLA2", "MGLL","LIPE", "LIPA", "LIPC", "PNLIP","LIPG","CEL","LIPF","LIPH", "DAGLB","DAGLA", "LIPI","LIPN","LIPM","LIPK","LIPJ",'F13A1')
...
ggplot(lipase.cal, aes(x=Genes, y=mean_Expression, fill=Disease))+
    geom_bar(stat="identity",width=.8, position=position_dodge(width=.8), col="black", show.legend=F) +
...
ggsave("../../data/Figures/Cushing-all-lipases-RPKM.pdf")
...
for (gene in lipases) {
  ...
  ggsave(filename=paste('../../data/Figures/Cushing-lipase-',gene,'-barplot.pdf',sep=""))
}
```
...
steroid_biosynthesis <- c('AKR1C1','DHCR7','DHCR24','HMGCS1', 'CYP1A1','CYP1B1','CYP7B1','CYP17A1','CYP19A1','SRD5A1','SRD5A3', 'NSDHL')
...
macrophage_markers <- c('NOS2','TNF','IL6','CXCL12','IL10','CLEC10A','ARG1','F13A1')
...
for (name in gene.data.name){
  ...
  ggplot(gene.data, aes(x=hgnc_symbol, y=mean_Expression, fill=Disease))+ 
    #this line together with show.legend=F and the element_rect=black get rid of the slash in legend box
  ...
  ggsave(filename=paste('../../data/Figures/Cushing-', name, '-barplot.pdf',sep=""))
}

```

```{r plot_with_different_axis_scale, echo=FALSE}
LHS <- c('AKR1C1','DHCR7','DHCR24','HMGCS1')
RHS <- c('CYP1A1','CYP1B1','CYP7B1','CYP17A1','CYP19A1','SRD5A1','SRD5A3')

steroid_biosynthesis <- c('AKR1C1','DHCR7','DHCR24','HMGCS1', 'CYP1A1','CYP1B1','CYP7B1','CYP17A1','CYP19A1','SRD5A1','SRD5A3')
index <- c(rep("scale 1",2), rep("scale 2", 10), rep("scale 1", 6), rep("scale 2",4))


require(ggplot2)
l.gene.data <- rpkm.long.cal[rpkm.long.cal$hgnc_symbol%in%eval(LHS),]
l.gene.data$hgnc_symbol <- factor(l.gene.data$hgnc_symbol, levels=eval(LHS))

r.gene.data <- rpkm.long.cal[rpkm.long.cal$hgnc_symbol%in%eval(RHS),]
r.gene.data$hgnc_symbol <- factor(r.gene.data$hgnc_symbol, levels=eval(RHS))

gene.data <- rpkm.long.cal[rpkm.long.cal$hgnc_symbol%in%eval(steroid_biosynthesis),]
gene.data$hgnc_symbol <- factor(gene.data$hgnc_symbol, levels=eval(steroid_biosynthesis))
...
ggsave(filename=paste('../../data/Figures/Cushing-twocord-steroidBiosyn-barplot.pdf',sep=""))
```
