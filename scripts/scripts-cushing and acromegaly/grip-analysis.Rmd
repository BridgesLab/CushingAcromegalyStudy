...
We therefore did Student's *t*-Tests and found that in the case of mean grip, at the end of the experiment we saw a `r with(subset(grip.summary, age=='158'), (Average[1]-Average[2])/Average[1])*100`% reduction in grip strength (p=`r mean.student$p.value`) and for median grip, we saw a `r with(subset(grip.summary, age=='158'), (Median[1]-Median[2])/Median[1])*100`% reduction in grip strength (p=`r median.student$p.value`).
