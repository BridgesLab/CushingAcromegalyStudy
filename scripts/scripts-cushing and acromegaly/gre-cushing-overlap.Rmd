...
Based on the data in Reddy *et al.*, we identified `r length(levels(gre.experimental.data$hgnc_symbol))` genes associated with GR CHIP peaks in A549 cells.  We found that `r length(intersect(gre.experimental.data$hgnc_symbol, sig.cushings.genes))` of these genes overlapped with the Cushings's differentially expressed genes.  These genes are shown in the Table below. 
...
Cushings <- c(length(intersect(yu.data$ENSEMBLE.gene.ID, sig.cushings.annotated)), dim(subset(cushings.annotated.data, padj<0.05&mmusculus_homolog_ensembl_gene!=''))[1])
Total <- c(length(unique(yu.data$ENSEMBLE.gene.ID)), dim(subset(cushings.annotated.data, mmusculus_homolog_ensembl_gene!=''))[1])
...
Among our Cushing's data, which is human we found  `r dim(subset(cushings.annotated.data, padj<0.05&mmusculus_homolog_ensembl_gene!=''))[1]` genes which had identifiable mouse homologs.  Within those genes, `r length(intersect(sig.cushings.annotated, yu.data$ENSEMBLE.gene.ID))` had CHIPseq peaks out of a total of `r length(unique(yu.data$ENSEMBLE.gene.ID))` peaks identified by Yu *et al.*.  This contingency table is shown below and showed enrichment of our genes in their data set (p=`r fisher.test(yu.summary.table)$p.value`).
