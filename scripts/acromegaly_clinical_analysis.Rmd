...
#for all measurements
outlier.all.o1 <- ddply(melted.data[melted.data$id!='1',], .(diagnosis, measurement), summarise, mean = mean(value), se=se(value))
outlier.all.o2 <- ddply(melted.data[melted.data$id!='1',], .(measurement), summarise, pval=t.test(value ~ diagnosis)$p.value)
...
outlier.all.summary.table <- merge(
  ...
  melted.data[melted.data$id=='1',c('measurement','value')],
  all=F
)
```
