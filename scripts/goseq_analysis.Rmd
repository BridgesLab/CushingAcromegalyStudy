...
This analysis removes the lowest 40% expressed transcripts, and also removes the data from patient 29.  This script was most recently run on `r date()`.
...
acromegaly.data <- read.csv('../data/processed/Annotated DESeq Results without Outlier - Acromegaly.csv', row.names='id')
cushing.data <- read.csv('../data/processed/Annotated DESeq Results without Outlier - Cushing.csv', row.names='id')
...
library(xtable)
print(xtable(annotated.GO.cushing.bp.unique[annotated.GO.cushing.bp.unique$padj<0.05,], digits=c(0,0,0,0,5,0), caption="Biological Processes Enriched in Cushing Patients"), type='html')
print(xtable(annotated.GO.cushing.mf.unique[annotated.GO.cushing.mf.unique$padj<0.05,], digits=c(0,0,0,0,5,0), caption="Molecular Functions Enriched in Cushing Patients"), type='html')
print(xtable(annotated.GO.cushing.kegg.unique[annotated.GO.cushing.kegg.unique$padj<0.05,], digits=c(0,0,0,0,5,0), caption="KEGG Pathways Enriched in Cushing Patientes"), type='html')
```
