...
cushing.mapping$age <- cut(cushing.mapping$age, breaks=c(0,40,60,100))
...
cushing.counts <- transcript.counts[,cushing.mapping.age40.60$samplename]
```
...
cushing.age40.60 <- cushing.protein.coding[cushing.protein.coding]
...
cds.cushing.age = DESeqDataSetFromMatrix(countData=cushing.protein.coding, colData=cushing.mapping.age40.60, design=~group)
...
plotMA(cushing.cds.age,ylim=c(-2,2),main="DESeq2 - Cushing adjusting for age")
...
cushing.results.age <- results(cushing.cds.age)
sum(acromegaly.results.age$padj<.05, na.rm=TRUE)
sum(cushing.results.age$padj<0.05, na.rm=TRUE)

write.csv(cushing.results.age, "../data/processed/cushing_results_ageGroup40to60.csv")
