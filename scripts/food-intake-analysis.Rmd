...
```{r food-intake-weekly-scatter}
#rplots scatter plot
ymax <- with(means.sem, (max(mean+sem)))
plot <- with(subset(means.sem, Treatment=="Control"&Week<=10), plot(Week, mean, las=1, pch=19, type="l", ylim=c(0,ymax), 
             ylab="Food Eaten Per Mouse Per Day (g)", xlab="Weeks of Dexamethasone"))
with(subset(means.sem, Treatment=="Dex"&Week<=10), lines(Week, mean, col="red", pch=19))

superpose.eb <- 
 function (x, y, ebl, ebu = ebl, length = 0.08, ...) 
 arrows(x, y + ebu, x, y - ebl, angle = 90, code = 3, 
 length = length, ...)
legend("bottomleft", c("Water",'Dexamethasone'), bty="n", lty=1, col=c('black','red'))
with(subset(means.sem, Treatment=="Control"&Week<=10), superpose.eb(Week,mean,sem))
with(subset(means.sem, Treatment=="Dex"&Week<=10), superpose.eb(Week,mean,sem, col="red"))
```
