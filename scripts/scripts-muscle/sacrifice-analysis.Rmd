...
filename <- '../../data/raw/HFD Dex Muscle Testing Data.xlsx'
  exp.data <- read_excel(filename, sheet = 'Sheet1') %>%
    ...
    mutate(Gastroc.rel = Gastroc/Weight*1000,
           Pct.Fat.Mass = Fat.Mass/Weight*100) %>%
...
summary.data <-
  exp.data %>%
  group_by(Sex,Diet, Treatment) %>%
  summarize_if(is.numeric, .funs=funs(mean.n,se,sd))
```
...
ggplot(data=summary.data,
       aes(y=Weight_mean.n/1000,
           ymin=(Weight_mean.n-Weight_se)/1000,
           ymax=(Weight_mean.n+Weight_se)/1000,
           x=Diet,
           fill=Treatment)) +
...
library(broom)
aov(`Weight` ~ Diet * Treatment, data = exp.data) %>% 
  tidy %>%
  kable(caption="ANOVA of Lean Mass")
...
summary.data %>% 
  ...
  pivot_wider(names_from='Type',
              values_from='Value') %>%
  kable(caption="Summary data for lean mass")
```
...
```{r lean-mass-barplot}
ggplot(data=summary.data,
       aes(y=Lean.Mass_mean.n/1000,
           ymin=(Lean.Mass_mean.n-Lean.Mass_se)/1000,
           ymax=(Lean.Mass_mean.n+Lean.Mass_se)/1000,
           x=Diet,
           fill=Treatment)) +
         geom_bar(stat='identity', beside=T, width=0.75, position='dodge') +
         geom_errorbar(position=position_dodge(width=0.75), width=0.5) +
...
library(broom)
aov(`Lean.Mass` ~ Diet * Treatment, data = exp.data) %>% 
  tidy %>%
  kable(caption="ANOVA of Lean Mass")

summary.data %>% 
  group_by(Sex,Diet,Treatment) %>%
  select(starts_with('Lean.Mass')) %>%
  pivot_longer(
    cols=starts_with('Lean.Mass'),
    names_to=c('Measurement','Type'),
    names_pattern = "(.*)_(.*)",
    values_to='Value') %>%
  pivot_wider(names_from='Type',
...
summary.data %>% 
  group_by(Sex,Diet,Treatment) %>%
  select(starts_with('Fat.Mass')) %>%
  pivot_longer(
    cols=starts_with('Fat.Mass'),
    names_to=c('Measurement','Type'),
    names_pattern = "(.*)_(.*)",
    values_to='Value') %>%
  pivot_wider(names_from='Type',
              values_from='Value') %>%
  kable(caption="Summary data for fat mass")

summary.data %>% 
  group_by(Sex,Diet,Treatment) %>%
  select(starts_with('Pct.Fat.Mass')) %>%
  pivot_longer(
    cols=starts_with('Pct.Fat.Mass'),
    names_to=c('Measurement','Type'),
    names_pattern = "(.*)_(.*)",
    values_to='Value') %>%
  pivot_wider(names_from='Type',
              values_from='Value') %>%
  kable(caption="Summary data for percent fat mass")
```
...
summary.data %>% select(Diet,Treatment,Gastroc_mean.n, Gastroc_se, Gastroc_sd) %>% mutate(Rel.Error = Gastroc_sd/Gastroc_mean.n*100) %>% kable(caption="Summary statistics for gastroc weights")
```
```{r gastroc-boxplot}
ggplot(data=exp.data,
       aes(y=Gastroc,
...
```{r rel-gastroc-barplot}
ggplot(data=summary.data,
       aes(y=Gastroc.rel_mean.n,
           ymin=Gastroc.rel_mean.n-Gastroc.rel_se,
           ymax=Gastroc.rel_mean.n+Gastroc.rel_se,
           x=Diet,
           fill=Treatment)) +
         geom_bar(stat='identity', beside=T, width=0.75, position='dodge') +
         geom_errorbar(position=position_dodge(width=0.75), width=0.5) +
          scale_fill_manual(values=c("black","red")) +
  expand_limits(y=0) +
  labs(title="Relative Gastrocnemius Mass",
       y="Weight (mg/g)",
       x="") +
  scale_fill_grey() +
  scale_x_discrete(labels=c("Lean","Obese")) +
...
aov(Gastroc.rel ~ Diet * Treatment, data = exp.data) %>% 
  tidy %>%
  kable(caption="ANOVA of Relative Gastroc Weights")

aov(Gastroc.rel ~ Diet + Treatment, data = exp.data) %>% 
  tidy %>%
  kable(caption="ANOVA of Relative Gastroc Weights with no interaction")
```
...
exp.data %>% 
  ...
  summarize(Mean = mean(`Po-Muscle (N)`),
            SE = se(`Po-Muscle (N)`),
            SD = sd(`Po-Muscle (N)`),
            n = length(`Po-Muscle (N)`),
            shapiro =shapiro.test(`Po-Muscle (N)`)$p.value) %>%
  mutate(Rel.SD = SD/Mean*100) %>%
  kable(caption='Summary statistics for muscle stimulation')

library(car)

bind_rows(leveneTest(`Po-Muscle (N)`~Treatment, data=exp.data %>% filter(Diet=="NCD")) %>% tidy(),
          (leveneTest(`Po-Muscle (N)`~Treatment, data=exp.data %>% filter(Diet=="HFD")) %>% tidy())) %>%
  kable(caption="Levene's tests for NCD and HFD respectively")

exp.data %>%
  group_by(Diet) %>%
  summarise(
        Student = t.test(`Po-Muscle (N)`[Treatment=="Water"],
                         `Po-Muscle (N)`[Treatment=="Dexamethasone"],
                         var.equal=T)$p.value,
    Welch = t.test(`Po-Muscle (N)`[Treatment=="Water"],
                     `Po-Muscle (N)`[Treatment=="Dexamethasone"],
                     var.equal=F)$p.value) %>%
  kable(caption="Pairwise significance testing for effects of dexamethasone on muscle-stimulated contraction")
...
exp.data %>% 
  group_by(Treatment,Diet) %>%
  summarize(Mean = mean(`Po-Nerve (N)`),
            SE = se(`Po-Nerve (N)`),
            SD = sd(`Po-Nerve (N)`),
            n = length(`Po-Nerve (N)`),
            shapiro =shapiro.test(`Po-Nerve (N)`)$p.value) %>%
  mutate(Rel.SD = SD/Mean*100) %>%
  kable(caption='Summary statistics for nerve stimulation')

library(car)

bind_rows(leveneTest(`Po-Nerve (N)`~Treatment, data=exp.data %>% filter(Diet=="NCD")) %>% tidy(),
          (leveneTest(`Po-Nerve (N)`~Treatment, data=exp.data %>% filter(Diet=="HFD")) %>% tidy())) %>%
  kable(caption="Levene's tests for NCD and HFD respectively for nerve stimulation")

exp.data %>%
  group_by(Diet) %>%
  summarise(
        Student = t.test(`Po-Nerve (N)`[Treatment=="Water"],
                         `Po-Nerve (N)`[Treatment=="Dexamethasone"],
                         var.equal=T)$p.value,
    Welch = t.test(`Po-Nerve (N)`[Treatment=="Water"],
                     `Po-Nerve (N)`[Treatment=="Dexamethasone"],
                     var.equal=F)$p.value) %>%
  kable(caption="Pairwise significance testing for effects of dexamethasone on nerve-stimulated contraction")
