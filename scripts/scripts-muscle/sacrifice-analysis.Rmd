...
filename <- 'data HFD DEX.xlsx' #make this a separate line, you can use any variable you want
library(forcats)

#this loads whatever the file is into a dataframe called exp.data if it exists
filename <- 'HFD Dex Muscle Testing Data.xlsx'
...
ggplot(data=summary.data,
       aes(y=Gastroc_mean.n,
         ...
         facet_grid(~Sex) +
  expand_limits(y=0) +
  labs(y="Average Gastrocnemius Weight (mg)")
library(broom)
...
ggplot(data=summary.data,
       aes(y=`Po-Muscle (mN)_mean.n`,
           ymin=`Po-Muscle (mN)_mean.n`-`Po-Muscle (mN)_se`,
           ymax=`Po-Muscle (mN)_mean.n`+`Po-Muscle (mN)_se`,
           x=Diet,
           fill=Treatment)) +
         geom_bar(stat='identity', beside=T, width=0.75, position='dodge') +
         geom_errorbar(position=position_dodge(width=0.75), width=0.5) +
...
plot.data <- summary.data %>% ungroup() %>% select(Diet,Treatment,`Po-Muscle (mN)_mean.n`) %>% spread(key="Diet",value="Po-Muscle (mN)_mean.n")
plot.data.se <- summary.data %>% ungroup() %>% select(Diet,Treatment,`Po-Muscle (mN)_se`) %>% spread(key="Diet",value="Po-Muscle (mN)_se")
plot <- barplot(as.matrix(plot.data[2:3]),
                beside=T,
                col=c('black','red'),
                las=1,
                ylim=c(0,6500),
                ylab="Muscle Force (mN)")
superpose.eb(plot,
             as.matrix(plot.data[2:3]),
             as.matrix(plot.data.se[2:3]))

library(broom)
aov(`Po-Muscle (mN)` ~ Diet * Treatment, data = exp.data) %>% tidy %>% kable(caption="ANOVA of Muscle force measurements")
```
...
library(broom)
aov(`Po-Muscle (mN)` ~ Diet * Treatment, data = exp.data) %>% 
  tidy %>%
...
ggplot(data=summary.data,
       aes(y=`Po-Nerve (mN)_mean.n`,
           ymin=`Po-Nerve (mN)_mean.n`-`Po-Nerve (mN)_se`,
           ymax=`Po-Nerve (mN)_mean.n`+`Po-Nerve (mN)_se`,
           x=Diet,
           fill=Treatment)) +
         geom_bar(stat='identity', beside=T, width=0.75, position='dodge') +
         geom_errorbar(position=position_dodge(width=0.75), width=0.5) +
...
library(broom)
aov(`Po-Nerve (mN)` ~ Diet * Treatment, data = exp.data) %>% tidy %>% kable(caption="ANOVA of Nerve force measurements")
plot <- barplot(as.matrix(plot.data[2:3]),
                beside=T,
                col=c('black','red'),
                las=1,
...
library(broom)
lm(`Po-Muscle (mN)` ~ CSA, data=exp.data) %>% tidy() %>% kable(caption="Linear model regressing against cross-sectional area")
lm(`Po-Muscle (mN)` ~ CSA, data=exp.data) %>% glance() %>% kable(caption="Linear model regressing against cross-sectional area, showing fit")
lm(`Po-Muscle (mN)` ~ CSA + Treatment, data=exp.data) %>% tidy() %>% kable(caption="Linear model including dexamethasone treatment as a covariate")
lm(`Po-Muscle (mN)` ~ CSA + Treatment, data=exp.data) %>% glance() %>% kable(caption="Linear model including dexamethasone treatment as a covariate, showing fit")
