...
```{r lean-mass-ggplot, fig.cap="Lean mass changes over five weeks"}
library(ggplot2)
library(forcats)
...
weight.summary %>%
  ...
  mutate(Diet = fct_recode(Diet, 
                           "Lean"="Normal Chow Diet",
                           "Obese"="High Fat Diet")) %>%
  mutate(Treatment = relevel(Treatment, ref="Water")) %>%
  mutate(Weeks=Time/7) %>%
  ggplot(aes(x=Weeks,
             y=Lean.Mass.mean,
             ymin=Lean.Mass.mean-Lean.Mass.se,
             ymax=Lean.Mass.mean+Lean.Mass.se,
         color=Treatment, lty=Diet)) +
  geom_point(size=3) +
...
endpoint.summary <-
  ...
  summarize_at(vars(Body.Weight:Percent.Fat.Mass), funs(mean,se,sd))

kable(endpoint.summary %>% select(Treatment, ends_with('mean')), caption="Endpoint summary data.")

kable(endpoint.summary %>% select(Treatment, ends_with('sd')), caption="Endpoint summary data for standard deviations.")
```
