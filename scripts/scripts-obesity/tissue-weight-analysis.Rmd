...
```{r quad-anova}
quad.aov.int <- aov(Quad ~ Diet + Group + Diet:Group, data=data)
quad.aov <- aov(Quad ~ Diet + Group, data=data)
```

```{r quad-levene}
levene.hfd.quad <- leveneTest(Quad ~ Group, data = filter(data, Diet=="HFD"))
levene.ncd.quad <- leveneTest(Quad ~ Group, data = filter(data, Diet=="NCD"))
```

The p-value from the Levene's test were `r levene.hfd.quad$"Pr(>F)"[1]` for HFD.  Based on this a Student's *t* test was performed with a p-value of **`r t.test(Quad ~ Group, data = filter(data, Diet=="HFD"), var.equal=T)$p.value`**.  

For NCD the Levene's test had a p-value of and  `r levene.ncd.quad$"Pr(>F)"[1]`, so equal variance could be assumed.  Based on this, a Student's *t*-test had a p-value of **`r t.test(Quad ~ Group, data = filter(data, Diet=="NCD"), var.equal=F)$p.value`** for NCD.

Based on a 2-way ANOVA with Diet and Group as the interacting covariates there was a significant interaction between diet and treatment:

```{r quad-anova-results}
kable(tidy(quad.aov.int), caption="Two Way ANOVA with Interaction between treatment and diet.")
```

The p-value for the interaction was **`r tidy(quad.aov.int)$p.value[3]`**.  The residuals of this model, passed through a Shapiro-Wilk test had a p-value of `r shapiro.test(residuals(quad.aov.int))$p.value`, so normality could not be assumed.

### Triceps Surae Statistics

```{r ts-anova}
ts.aov.int <- aov(TS ~ Diet + Group + Diet:Group, data=data)
ts.aov <- aov(TS ~ Diet + Group, data=data)
```

```{r ts-levene}
levene.hfd.ts <- leveneTest(TS ~ Group, data = filter(data, Diet=="HFD"))
levene.ncd.ts <- leveneTest(TS ~ Group, data = filter(data, Diet=="NCD"))
```

The p-value from the Levene's test were `r levene.hfd.ts$"Pr(>F)"[1]` for HFD.  Based on this a Student's *t* test was performed with a p-value of **`r t.test(TS ~ Group, data = filter(data, Diet=="HFD"), var.equal=T)$p.value`**.  

For NCD the Levene's test had a p-value of and  `r levene.ncd.ts$"Pr(>F)"[1]`, so equal variance could be assumed.  Based on this, a Student's *t*-test had a p-value of **`r t.test(TS ~ Group, data = filter(data, Diet=="NCD"), var.equal=F)$p.value`** for NCD.

Based on a 2-way ANOVA with Diet and Group as the interacting covariates there was a significant interaction between diet and treatment:

```{r ts-anova-results}
kable(tidy(ts.aov.int), caption="Two Way ANOVA with Interaction between treatment and diet.")
```

The p-value for the interaction was **`r tidy(ts.aov.int)$p.value[3]`**.  The residuals of this model, passed through a Shapiro-Wilk test had a p-value of `r shapiro.test(residuals(ts.aov.int))$p.value`, so normality could not be assumed.

## Overall Reductions in NCD/HFD

```{r percent-reductions}
summary.data %>%
  select(ends_with('mean.na'),Treatment) %>%
  group_by(Diet,Treatment) %>%
  gather(key=Measure, value=Amount, -Diet, -Treatment) %>%
