...
sheetname <- 'Exported'
library(readxl)
combined_data <- read_excel(filename, sheet= sheetname)
...
tg.summary <-
  ...
  summarize(Glycerol.mean = mean(Glycerol, na.rm=T),
            ...
            Glycerol.sd = sd(Glycerol),
            Glycerol.n = length(Glycerol),
            Rel.error = Glycerol.sd/Glycerol.mean*100)

kable(tg.summary, caption="Summary statistics by group")
```
