...
blot.data <-
  data %>%
  group_by(Experiment) %>%
  mutate(Normalized.Levels = ATGL.Revert/Revert) %>%
...
The relative data could be assumed to be normally distributed via Shapiro-Wilk test (p=`r shapiro.test(filter(blot.data, Treatment=="Dexamethasone")$Relative.Levels)$p.value`), so we did a one-way *t*-test relative to a value of 1.  This yielded a p-value of **`r t.test(filter(blot.data, Treatment=="Dexamethasone")$Relative.Levels, mu=1)$p.value`**
