...
```{r data-entry}
filename <- '../../data/raw/HFD vs NCD ATGL Blot Quantification.csv'
library(readr)
...
blot.data <-
  ...
  select(Diet,Treatment,ATGL.norm) %>%
  mutate(Relative.Levels=ATGL.norm/mean(ATGL.norm[Diet=="Normal Chow Diet"&Treatment=="Water"])) 
...
```{r tukey-table}
kable(tg.tukey$`Diet:Treatment`, caption="Tukey's HSD tests for iWAT ATGL Levels")
```
