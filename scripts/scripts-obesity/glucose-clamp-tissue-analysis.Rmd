...
library(ggplot2)
select(summary,Treatment,Heart_mean, Heart_se) %>%
  #pivot_longer(cols=2:3, names_to="Measure") %>%
  ggplot(aes(y=Heart_mean,
             ymin=Heart_mean-Heart_se,
             ymax=Heart_mean+Heart_se,
             x=Treatment)) +
  labs(y="2-DG Uptake (nmol/g/min)",
       title="Cardiac Glucose Uptake",
       subtitle="(In Addition to Cardiac Atrophy)",
       x="")  +
