...
n.stat <- ddply(subset(melted.data.used, measurement=='BMI'), .(diagnosis, BMI.cat), summarise, n = length(value))
kable(n.stat, caption="Subjects in each group")
...
Cushing's disease resulted in a **`r subset(clinical.data, BMI.cat=='Obese')$mean[2]/subset(clinical.data, BMI.cat=='Obese')$mean[1]`** fold increase in HOMA-IR score in the obese subejcts but only a **`r subset(clinical.data, BMI.cat=='Not Obese')$mean[2]/subset(clinical.data, BMI.cat=='Not Obese')$mean[1]`** fold increase in the non-obese subjects.
...
```{r clinical-barplots-cushing-ast}
item <- 'AST'
  #pdf(sprintf('../figures/Cushing-%s-barplot.pdf', gene))
  clinical.data <- all.o1[all.o1$measurement==item,]
  ggplot(clinical.data, aes(x=diagnosis, y=mean, fill=BMI.cat)) + 
...
```
```{r clinical-barplots-cushing-alt}
item <- 'ALT'
  #pdf(sprintf('../figures/Cushing-%s-barplot.pdf', gene))
  clinical.data <- all.o1[all.o1$measurement==item,]
  ggplot(clinical.data, aes(x=diagnosis, y=mean, fill=BMI.cat)) + 
...
Cushing's disease resulted in a **`r subset(clinical.data, BMI.cat=='Obese')$mean[2]/subset(clinical.data, BMI.cat=='Obese')$mean[1]`** fold increase in ALT levels in the obese subejcts but only a **`r subset(clinical.data, BMI.cat=='Not Obese')$mean[2]/subset(clinical.data, BMI.cat=='Not Obese')$mean[1]`** fold increase in the non-obese subjects.
