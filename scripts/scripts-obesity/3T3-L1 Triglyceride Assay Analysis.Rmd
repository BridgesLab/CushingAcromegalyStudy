...
data.norm <-
  data %>%
  group_by(Experiment) %>%
  mutate(TG = TG/TG[Treatment=="Mature Adipocytes"]) 
...
```{r 3T3-L1-TG-Levels-normalized}
plot <- with(summary.norm, barplot(mean,
                              las=1,
                              names.arg=Treatment,
                              ylim=c(0,max(mean+se)),
                              ylab="Triglyceride Levels (Relative Units)"))

superpose.eb(plot,summary.norm$mean, summary.norm$se)

kable(summary.norm, caption="Normalized values")
```
...
For evaluating the effects of dexamethasone, after normalizing to each experiment's control, we observed that dexamethasone reduced TG levels by **`r (summary.norm$mean[2]-summary.norm$mean[3])/summary.norm$mean[2]*100`%**.


To test for significance, we first did a Shapiro-Wilk test on the two groups.  This yielded p-value of `r shapiro.test(filter(data.norm, Treatment=='Mature Adipocytes \n+ Dexamethasone')$TG)$p.value` for the cells treated with Dexamethasone.  We therefore did a One-way *t*-test.  This had a p-value of **`r t.test(filter(data.norm, Treatment=='Mature Adipocytes \n+ Dexamethasone')$TG, mu=1)$p.value`**.
